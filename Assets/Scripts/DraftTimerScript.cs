...
public class DraftTimerScript : MonoBehaviour
{
	...
	public AudioClip[] DrafterAudio = new AudioClip[12];
	...
	void Start()
	{
		...
		for (int i = 0; i < (int)DrafterEnum.TotalDrafters; ++i)
		{
			...
			playerProfiles[i].playerEnum = (DrafterEnum)i;
	...
	public void BeginDraft()
	{
		...
		GameObject.Find("MusicController").GetComponent<MusicController>().StartMusic();
		UpdateLabels();
	}
	...
	public void StartMainDraft()
	{
		...
		foreach (DrafterEnum drafter in DraftOrder)
		{
			...
			if (contracts < 3)
			{
				// Compensation pick for 2 year contracts (after round 5)
				if (playerProfiles[(int)drafter].twoYearContracts.Count == 0)
				{
					...
					newPick.pickNumber = pickInfo[4].Count;

					pickInfo[4].Add(newPick);
				}

				// Compensation pick for 1 year contracts (after round 4)
				if (playerProfiles[(int)drafter].oneYearContracts.Count == 0)
				{
					PickInfo newPick = new PickInfo();
					newPick.drafterID = drafter;
					newPick.roundNumber = 3;
					newPick.pickNumber = pickInfo[3].Count;

					pickInfo[3].Add(newPick);
				}
			}
		}
		...
		GameObject.Find("BestAvailablePauseButton").GetComponent<BestAvailablePauseButton>().Show();
		GameObject.Find("AlreadyPickedPauseButton").GetComponent<AlreadyPickedPauseButton>().Show();
		...
		GameObject.Find("MusicController").GetComponent<MusicController>().StopMusic();
	...
	public GameObject PickAnnouncer = null;
	...
	private void AnimateToNextDrafter()
	{
		...
		if (animationInitialized)
		{
			...
			if (newDrafterNameplate != null)
			{
				...
				PickAnnouncer.GetComponent<AudioSource>().clip = DrafterAudio[(int)pickInfo[currentRound][currentPick].drafterID];
			}
			...
			if (currentAnimationTime >= animationTime)
			{
				...
				PickAnnouncer.GetComponent<AudioSource>().Play();
			}
		}
		else
		{
			...
			if (oldDrafterNameplate)
			{
				oldDrafterNameplate.transform.DOMoveX(endingNameplateX, quickAnimationTime).SetEase(Ease.InQuad);
			}
			...
			foreach (PlayerProfile profile in playerProfiles)
			{
				...
				if (profile.playerName == contractFields[0])
				{
					foreach (string contract in contractFields)
					{
						...
						{
							profile.oldThreeYearContracts.Add(contract.TrimEnd('*', '3').Replace(".", string.Empty));
						}
						else if (contract.Contains("*2"))
						{
							profile.oldTwoYearContracts.Add(contract.TrimEnd('*', '2').Replace(".", string.Empty));
						}
						else if (contract.Contains("*1"))
						{
							profile.oldOneYearContracts.Add(contract.TrimEnd('*', '1').Replace(".", string.Empty));
						}
	...
	public void StartPostDraftStats()
	{
		...
		GameObject.Find("BestAvailablePauseButton").GetComponent<BestAvailablePauseButton>().Hide();
		GameObject.Find("AlreadyPickedPauseButton").GetComponent<AlreadyPickedPauseButton>().Hide();
